...
export default class AlsoBought {
    constructor(parentProductDetails, {
        ...
        const thumbSize = this.context.themeSettings.productthumb_size.split('x');
            ...
            $.ajax({
                ...
                success: (resp) => {
                    resp.data.site.products.edges.forEach(({ node }) => {
                        ...
                        const priceLabel = this.context.themeSettings['pdp-price-label'];
                        const salePriceLabel = this.context.themeSettings['pdp-sale-price-label'];
                        const nonSalePriceLabel = this.context.themeSettings['pdp-non-sale-price-label'];
                        const retailPriceLabel = this.context.themeSettings['pdp-retail-price-label'];
                        ...
                        const checked = this.context.themeSettings.alsobought_checked;
    ...
    async loadProductDetails($productEl) {
        ...
        await new Promise(resolve => {
            utils.api.product.getById(productId, { template: 'chiara/also-bought/product-details' }, (err, resp) => {
                if (err || !resp) {
                    return;
                }
