...
export default class ProductDetails {
    constructor($scopeArg, context, productAttributesData = {}, loadProductAttributes = false) { // Papathemes Also Bought MOD
        ...
        this.imageGallery = new ImageGallery($('[data-image-gallery]', this.$scope), context); // papathemes-chiara: add context
        ...
        if (context.enableAlsoBought) {
            try {
                this.alsoBought = new AlsoBought(this, this.context.chiaraSettings.alsoBoughtOptions);
            } catch (err) {
        ...
        if ((_.isEmpty(productAttributesData) || hasDefaultOptions) && (hasOptions || loadProductAttributes)) { // papathemes-chiara edited for also-bought
        ...
        if (this.context.themeSettings.swatch_option_display_type === 'dropdown' || this.context.themeSettings.swatch_option_display_type === 'both') {
            import('../../chiara/dropdown-swatches')
                .then(module => module.default(this, {
                    hideOriginalSwatches: this.context.themeSettings.swatch_option_display_type === 'dropdown',
                    ...(this.context.chiaraSettings.dropdownSwatchesOptions || {}),
                }));
        }
    }
        ...
        try {
            ...
            $.each($('[data-product-attribute]'), (index, value) => {
                ...
                if (type === 'set-rectangle' || type === 'set-radio' || type === 'swatch' || type === 'input-checkbox' || type === 'product-list') {
                    const checked = value.querySelector('input:checked'); // papathemes-chiara edited
                    if (checked) {
                        if (type === 'set-rectangle' || type === 'set-radio' || type === 'product-list') {
                            const label = checked.labels ? checked.labels[0].innerText : value.querySelector(`label[for="${checked.id}"]`).innerText; // papathemes-chiara fix js error on IE11
                            if (label) {
                        ...
                        if (type === 'swatch') {
                            const label = checked.labels ? checked.labels[0].children[0] : value.querySelector(`label[for="${checked.id}"]`).children[0]; // papathemes-chiara fix js error on IE11
                            if (label) {
            ...
            if (productVariant) {
                ...
                if (view.attr('data-event-type')) {
                    ...
                    if (el) {
                        ...
                        const card = $(`[data-name="${productName.replace(/"/g, '\\"')}"]`);
    ...
    addProductToCartAsync() {
        return new Promise(resolve => {
            ...
            const $buyNowBtn = $('#form-action-buyNow', this.$scope);
    ...
    addProductToCart(event, form) {
        ...
        const $buyNowBtn = $('#form-action-buyNow', form); // papathemes-chiara
        const originalBtnVal = $addToCartBtn.val();
        const waitMessage = $addToCartBtn.data('waitMessage');
        ...
        $addToCartBtn
            .val(waitMessage)
            .prop('disabled', true);
        $buyNowBtn.prop('disabled', true); // papathemes-chiara
        ...
        utils.api.cart.itemAdd(this.filterEmptyFilesFromForm(new FormData(form)), async (err, response) => { // AlsoBought Mod
            ...
            $addToCartBtn
                .val(originalBtnVal)
                .prop('disabled', false);
            $buyNowBtn.prop('disabled', false); // papathemes-chiara
