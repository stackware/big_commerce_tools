...
export class Modal {
    constructor($modal, {
        ...
        this.$modal.on('click', '.dropdown-menu-button, [data-dropdown]', e => {
            e.stopPropagation();
        });
    }
    ...
    updateContent(content, { wrap = false } = {}) {
        ...
        // Chiara fix blurry popup
        const marginTop = this.$modal.offset().top % 1;
    ...
    onModalClosed() {
        ...
        if (/Mobi/i.test(navigator.userAgent) && /Safari/i.test(navigator.userAgent)) {
            const el = this.$content.find('[data-lock-body-scroll]')[0];
            if (el) {
                enableBodyScroll(el);
            }
        }
